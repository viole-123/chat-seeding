...
func (r *RateLimit) CheckHourlyEventLimit(ctx context.Context, matchID string, maxEventsPerHour int) (bool, error) {
	...
	key := fmt.Sprintf("ratelimit:hourly_events:%s:%s", matchID, hourBucket)
	...
	if counter >= int64(maxEventsPerHour) {
		log.Printf("⏭️  [RATELIMIT-HOURLY] blocked: match=%s used=%d limit=%d bucket=%s", matchID, counter, maxEventsPerHour, hourBucket)
		return false, nil
	}

	ttl := now.Truncate(time.Hour).Add(time.Hour).Sub(now)
	if ttl <= 0 {
		ttl = time.Hour
	}

	if err := r.incrementCounter(ctx, key, ttl); err != nil {
		return false, err
	}

	log.Printf("✅ [RATELIMIT-HOURLY] pass: match=%s used=%d next=%d limit=%d bucket=%s", matchID, counter, counter+1, maxEventsPerHour, hourBucket)
	return true, nil
}
