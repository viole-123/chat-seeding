...
func Bootstrap() error {
	...
	roomManager := seeding.NewRoomManager(redisClient, 6*time.Hour)
	...
	botReplySystem := seeding.NewBotReplySystem(intentDetector, llmGateway, personaSelector, rateLimitService)
	...
	gatewayURL := strings.TrimSpace(os.Getenv("WS_GATEWAY_URL"))
	if gatewayURL == "" {
	...
	handler.SetUserMessageHandler(func(in handler.IncomingUserMessage) {
		...
		_ = rateLimitService.IncrTotalMessages(context.Background(), matchID)

		go func(mID, rID string, uMsg model.UserMessage) {
			bundle, err := contextBuilder.BuildBundle(context.Background(), mID, rID)
			if err != nil || bundle == nil {
				bundle = &model.ContextBundle{Match: model.MatchState{MatchID: mID}}
			}

			// Đưa user message vào đầu history để LLM thấy ngữ cảnh mới nhất
			bundle.Chat.RawMessages = append(
				[]model.ChatMessage{{
					Content:   uMsg.Content,
					Timestamp: time.Now().Unix(),
					IsBot:     false,
				}},
				bundle.Chat.RawMessages...,
			)

			// FIX: context độc lập, defer cancel đúng scope (goroutine này)
			replyCtx, cancelReply := context.WithTimeout(context.Background(), 8*time.Second)
			defer cancelReply()

			reply, err := botReplySystem.GenerateReply(replyCtx, uMsg, *bundle)
			...
			if mqttPublisher != nil {
				...
				if err := mqttPublisher.PublishBotMessage(context.Background(), rID, mqttMsg); err != nil {
					log.Printf("⚠️  [BOT-REPLY] mqtt publish failed: %v", err)
					// FIX: non-fatal — KHÔNG return ở đây, vẫn phải save DB/Redis
				}
			} else {
			...
			// Save sau publish — dù publish fail vẫn phải lưu
			_ = contextStore.PushChatMessage(rID, botMsg)
			if messageRepo != nil {
				_ = messageRepo.SaveMessage(botMsg)
			}
			_ = rateLimitService.IncrTotalMessages(context.Background(), mID)

			log.Printf("✅ [BOT-REPLY] room=%s persona=%s type=%s %dms",
				rID, reply.PersonaID, reply.ReplyType, reply.LatencyMs)

		}(matchID, roomID, model.UserMessage{Content: in.Content}) // pass by value, không capture
	})
	...
	router.POST("/api/chat/send", func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&in); err != nil || in.MatchID == "" || in.Content == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "match_id and content are required"})
			return
		}

		// FIX: dùng SetUserMessageHandler callback thay vì publish trực tiếp
		// để đi qua toàn bộ pipeline (save DB, bot reply, rate limit...)
		handler.TriggerUserMessage(in)
