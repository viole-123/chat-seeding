...
func (h *EventHandler) updateMatchState(event model.MatchEvent, daily *model.MatchDailyCatchFromRedis) error {
	...
	competitionName := event.LeagueID

	if daily != nil {
		homeTeam = daily.HomeTeam
		awayTeam = daily.AwayTeam
		competitionName = daily.Competition.Name
		if competitionName == "" {
			competitionName = daily.Competition.ID
		}
		...
		h.logger.Printf("│ │  🔍 ENRICH   [%s vs %s] phase=%s league=%s",
			homeTeam.ShortName, awayTeam.ShortName, phase, competitionName)
	}
...
func (h *EventHandler) enrichEventFromDaily(event *model.MatchEvent, daily *model.MatchDailyCatchFromRedis) {
	if daily == nil {
		return
	}
...
func (h *EventHandler) handleEvent(ctx context.Context, index int, total int, event model.MatchEvent, source string) error {
	...
	if err := h.updateMatchState(event, daily); err != nil {
