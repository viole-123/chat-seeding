...
func (p *PolicyChecker) CheckPolicy(ctx context.Context, event model.MatchEvent) (bool, error) {
	...
	allowed, err = p.rateLimitService.CheckHourlyEventLimit(ctx, event.MatchID, p.seedingPolicy.MaxEventsPerHour)
	if err != nil {
		return false, fmt.Errorf("check hourly event limit failed: %w", err)
	}
	if !allowed {
		log.Printf("   ⏭️  [POLICY] skip by hourly event limit: match=%s max=%d", event.MatchID, p.seedingPolicy.MaxEventsPerHour)
		return false, nil
	}
	log.Printf("   ✅ [POLICY] hourly event limit pass: match=%s", event.MatchID)
...
func (p *PolicyChecker) validateEventData(event model.MatchEvent) bool {
	if event.LeagueID == "" {
		log.Printf("[POLICY] invalid event: league_id is empty match=%s type=%s", event.MatchID, event.Type)
		return false
	}
...
func exceedsProjectedBotRatio(messages []model.ChatMessage, maxRatio float64) (bool, int, int, float64) {
	...
	// No user traffic yet: allow a small warm-up burst only, then block to avoid bot-only spam.
	if userCount == 0 {
		const maxBotWithoutUser = 3
		projected := float64(botCount+1) / float64(botCount+1)
		return botCount+1 > maxBotWithoutUser, botCount, userCount, projected
	}
